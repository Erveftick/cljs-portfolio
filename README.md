...
* [`src/cljs_portfolio/`](src/cljs_portfolio/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs_portfolio/core.cljs): contains the SPA entry point, `init`
...
5. Clone this repo and open a terminal in the `cljs-portfolio` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/cljs_portfolio/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns cljs-portfolio.example
  (:require [cljs-portfolio.config :as config])
